...
const FULL_DATABASE = {
  'soccer_uefa_champions_league': [
    // PARTIDOS DE HOY 10.03
    { id: 'ucl_h1', date: '10.03. 21:00', home: 'Milan', away: 'Benfica', oL: 2.15, oE: 3.40, oV: 3.40 },
    { id: 'ucl_h2', date: '10.03. 21:00', home: 'Monaco', away: 'Arsenal', oL: 4.20, oE: 3.75, oV: 1.85 },
    // MAÑANA Y RESTO
...
export default function GolpredictPro() {
  ...
  const comboActual = getCombo(activeTab === 'ia combos' ? 'Moderada' : 'Moderada');
    ...
    <div style={{background:'#000', color:'#fff', minHeight:'100vh', fontFamily:'monospace', maxWidth:'480px', margin:'0 auto', paddingBottom:'80px'}}>
      ...
      <div style={{display:'flex', background:'#080808', borderBottom:'1px solid #222'}}>
        {['partidos', 'ia combos', 'calculadora'].map(t => (
          <button key={t} onClick={() => setActiveTab(t)} style={{flex:1, padding:'15px', background:'none', border:'none', borderBottom: activeTab === t ? '3px solid #fbbf24' : 'none', color: activeTab === t ? '#fbbf24' : '#555', fontSize:'0.7rem', fontWeight:'bold'}}>{t.toUpperCase()}</button>
        ))}
      </div>
      ...
      <div style={{padding:'15px'}}>
        ...
        {activeTab === 'partidos' && analysedDb[liga]?.map(p => (
          <div key={p.id} style={{background:'#0c0c0c', padding:'20px', borderRadius:'20px', marginBottom:'15px', border:'1px solid #1a1a1a'}}>
            ...
            <div style={{fontSize:'0.95rem', fontWeight:'900', marginBottom:'15px', color:'#fff', textAlign:'center'}}>
              {p.home} <span style={{color:'#fbbf24', opacity:0.8}}>VS</span> {p.away}
            </div>
            <div style={{display:'flex', gap:'8px'}}>
              {[ {q:p.oL, p:p.pL, v:p.vL, t:'1'}, {q:p.oE, p:p.pE, v:p.vE, t:'X'}, {q:p.oV, p:p.pV, v:p.vV, t:'2'} ].map((o, i) => (
                <div key={i} style={{flex:1, background: o.v ? 'rgba(74,222,128,0.1)' : '#151515', border: o.v ? '2px solid #4ade80' : '1px solid #333', padding:'12px 0', borderRadius:'15px', textAlign:'center'}}>
                  ...
                  <div style={{fontSize:'0.6rem', color: o.v ? '#4ade80' : '#666'}}>{o.t} | {o.p}%</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'ia combos' && ['Sencilla', 'Moderada', 'Arriesgada'].map(r => {
          ...
          return (
            <div key={r} style={{background:'#0c0c0c', padding:'20px', borderRadius:'25px', marginBottom:'15px', border:'1px solid #222'}}>
              ...
              {c.sel.map((s,i) => (
                <div key={i} style={{fontSize:'0.75rem', color:'#fff', margin:'8px 0', display:'flex', justifyContent:'space-between'}}>
                  <span>{s.home} - {s.away}</span>
                  <span style={{color:'#fbbf24', fontWeight:'bold'}}>Pick: {s.pick}</span>
                </div>
              ))}
            </div>
          );
        ...
        {activeTab === 'calculadora' && (
          <div style={{background:'#0c0c0c', padding:'30px', borderRadius:'25px', textAlign:'center', border:'1px solid #fbbf24'}}>
            ...
            <div style={{fontSize:'0.7rem', color:'#666'}}>CUOTA TOTAL (IA MODERADA)</div>
            <div style={{fontSize:'1.5rem', fontWeight:'bold', margin:'10px 0', color:'#4ade80'}}>@{ comboActual.totalOdd.toFixed(2) }</div>
            <div style={{background:'#fbbf24', color:'#000', padding:'20px', borderRadius:'15px', marginTop:'20px'}}>
              ...
              <div style={{fontSize:'2.2rem', fontWeight:'900'}}>{ (betAmount * comboActual.totalOdd).toFixed(2) }€</div>
            </div>
          </div>
        )}
      </div>
