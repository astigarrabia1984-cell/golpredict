...
/* ===========================
LÓGICA IA Y SISTEMA DE PUNTUACIÓN
=========================== */
const teamRatings = {
...
function runModel(match) {
  ...
  // Validación de resultados (Verde/Rojo)
  let status = "pending"; 
  if (match.ftScore) {
    const [hG, aG] = match.ftScore.split("-").map(Number);
    const actualRes = hG > aG ? "1" : (hG === aG ? "X" : "2");
    status = actualRes === bestPick ? "hit" : "miss";
  }

  // Ajuste Realista: Córners y Tarjetas
    ...
    <div style={{ background: "#111", padding: "12px", borderRadius: "8px", marginBottom: "10px", border: `2px solid ${borderColor}`, cursor: "pointer" }} onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div style={{ marginTop: "10px", padding: "8px", background: "#050505", borderRadius: "5px", fontSize: "0.75em", borderTop: "1px solid #222" }}>
          ...
          <div>Córners: <b style={{color: "#ffaa00"}}>{data.corners}</b></div>
          <div>Tarjetas: <b style={{color: "#ff3333"}}>{data.cards}</b></div>
        </div>
      )}
    </div>
  );
...
export default function GolPredictPro() {
  ...
  return (
    <div style={{ background: "#0a0a0a", color: "#e0e0e0", minHeight: "100vh", padding: "10px", fontFamily: "sans-serif" }}>
      <h2 style={{ color: "#00ff41", textAlign: 'center', letterSpacing: "1px" }}>GOLPREDICT PRO</h2>

      <nav style={{ display: "flex", gap: "5px", marginBottom: "15px" }}>
        <button onClick={() => setActiveTab("partidos")} style={{ flex: 1, padding: "12px", background: activeTab === "partidos" ? "#00ff41" : "#222", border: "none", color: activeTab === "partidos" ? "#000" : "#fff", borderRadius: "5px", fontWeight: "bold" }}>LIGAS</button>
        <button onClick={() => setActiveTab("combo")} style={{ flex: 1, padding: "12px", background: activeTab === "combo" ? "#ffaa00" : "#222", border: "none", color: activeTab === "combo" ? "#000" : "#fff", borderRadius: "5px", fontWeight: "bold" }}>⭐ SUPER COMBO</button>
      </nav>
      ...
      {activeTab === "combo" && (
        <div style={{ maxWidth: "500px", margin: "0 auto", background: "#111", padding: "20px", borderRadius: "15px", border: "2px solid #ffaa00" }}>
          ...
          <p style={{ fontSize: "0.8em", color: "#888", textAlign: "center" }}>Selección de alta probabilidad (España, UK, Italia, Alemania)</p>
          {superCombo.length > 0 ? superCombo.map(m => (
            <div key={m.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #222" }}>
              ...
              <span style={{ color: "#00ff41", fontWeight: "bold", fontSize: "0.85em" }}>Gana {m.res.bestPick} ({m.res.bestProb}%)</span>
            </div>
          )) : <p style={{textAlign:"center", color:"#666"}}>No hay partidos disponibles para hoy</p>}
          {superCombo.length > 0 && (
            <div style={{ marginTop: "20px", textAlign: "center", padding: "15px", background: "#00ff41", color: "#000", borderRadius: "8px", fontWeight: "bold" }}>
              Confianza IA: {(superCombo.reduce((acc, m) => acc * (m.res.bestProb/100), 1) * 100).toFixed(1)}%
            </div>
          )}
        </div>
      )}
