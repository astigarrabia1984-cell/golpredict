...
const getS = (h, a, t) => {
  ...
  return { ...res, p1, pX, p2, g: isN ? h : sum.toFixed(1), cv: isN ? Math.round(h/5) : Math.round(sum+6) };
};
    ...
    <div style={{ background: "#000", color: "#FFF", minHeight: "100vh", padding: "15px", fontFamily: "sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "20px", overflowX: "auto", whiteSpace: "nowrap" }}>
        {Object.keys(DATA).map(l => (
          <button key={l} onClick={() => {setTab(l); setOpen(null);}} style={{ padding: "10px 15px", borderRadius: "8px", border: "none", background: tab === l ? "#00ff41" : "#222", color: tab === l ? "#000" : "#FFF", fontWeight: "bold", fontSize: "0.7rem" }}>{l}</button>
        ))}
      </div>
      ...
      {DATA[tab].map(m => {
        ...
        return (
          <div key={m.id} style={{ background: "#111", borderRadius: "12px", marginBottom: "10px", border: isOpen ? "1px solid " + s.col : "1px solid #333", overflow: "hidden" }}>
            ...
            {isOpen && (
              <div style={{ padding: "15px", background: "#080808", borderTop: "1px solid #222" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", marginBottom: "15px", textAlign: "center" }}>
                  <div><div style={{fontSize:"0.6rem", color:"#555"}}>LOCAL</div><div style={{fontWeight:"bold"}}>{s.p1}%</div></div>
                  <div><div style={{fontSize:"0.6rem", color:"#555"}}>EMPATE</div><div style={{fontWeight:"bold"}}>{tab === "NBA" ? "-" : s.pX+"%"}</div></div>
                  <div><div style={{fontSize:"0.6rem", color:"#555"}}>VISIT.</div><div style={{fontWeight:"bold"}}>{s.p2}%</div></div>
                </div>
                <div style={{ height: "6px", background: "#222", borderRadius: "3px", display: "flex", overflow: "hidden", marginBottom: "15px" }}>
                  <div style={{ width: s.p1 + "%", background: "#00ff41" }}></div>
                  {tab !== "NBA" && <div style={{ width: s.pX + "%", background: "#ffa500" }}></div>}
                  <div style={{ width: s.p2 + "%", background: "#ff4444" }}></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                  ...
                  <div style={{ background: "#1a1a1a", padding: "10px", borderRadius: "8px", textAlign: "center" }}>
                    <div style={{ fontSize: "0.6rem", color: "#00ff41" }}>{tab === "NBA" ? "ASISTENCIAS" : "CORNERS"}</div>
                    <div style={{ fontSize: "1.1rem", fontWeight: "bold" }}>{s.cv}</div>
                  </div>
                </div>
              </div>
            )}
