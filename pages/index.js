  ...
  if (!user) {
    return (
      <div style={{ textAlign: 'center', backgroundColor: '#000', color: '#fff', height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <button onClick={login} style={{ padding: '15px 30px', cursor: 'pointer', margin: 'auto', backgroundColor: '#fff', color: '#000', fontWeight: 'bold', border: 'none', borderRadius: '5px' }}>
          ENTRAR CON GOOGLE
        </button>
      </div>
    );
          ...
          {loading ? (
            ...
            <div>
              {partidos.map((p, i) => {
                ...
                return (
                  <div key={i} style={{ border: '1px solid #333', padding: '15px', marginBottom:
